...
OIDC login is currently available in the web and CLI browser flows. Desktop and mobile native handoff remain disabled for OIDC until the callback can use a claimed universal link and a short-lived, verifier-bound exchange code instead of placing a reusable session token in a custom-scheme URL.
