...
UPDATE issue SET
    ...
    last_activity_at = CASE
        WHEN metadata IS DISTINCT FROM metadata - $1::text
...
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, last_activity_at FROM issue
WHERE workspace_id = $1
...
SELECT i.id, i.workspace_id, i.title, i.description, i.status, i.priority, i.assignee_type, i.assignee_id, i.creator_type, i.creator_id, i.parent_issue_id, i.acceptance_criteria, i.context_refs, i.position, i.due_date, i.created_at, i.updated_at, i.number, i.project_id, i.origin_type, i.origin_id, i.first_executed_at, i.start_date, i.metadata, i.stage, i.properties, i.last_activity_at FROM issue i
WHERE i.workspace_id = $1
...
UPDATE issue SET
    ...
    last_activity_at = CASE
        WHEN metadata IS DISTINCT FROM jsonb_set(metadata, ARRAY[$1::text], $2::jsonb)
        THEN GREATEST(COALESCE(last_activity_at, updated_at), now())
        ELSE last_activity_at
    END,
    updated_at = now()
...
UPDATE issue SET
    ...
    last_activity_at = CASE WHEN
        title IS DISTINCT FROM COALESCE($2, title)
        OR description IS DISTINCT FROM COALESCE($3, description)
        OR status IS DISTINCT FROM COALESCE($4, status)
        OR priority IS DISTINCT FROM COALESCE($5, priority)
        OR assignee_type IS DISTINCT FROM $6
        OR assignee_id IS DISTINCT FROM $7
        OR start_date IS DISTINCT FROM $9
        OR due_date IS DISTINCT FROM $10
        OR parent_issue_id IS DISTINCT FROM $11
        OR project_id IS DISTINCT FROM $12
        OR stage IS DISTINCT FROM $13
        THEN GREATEST(COALESCE(last_activity_at, updated_at), now())
        ELSE last_activity_at
    END,
    updated_at = now()
